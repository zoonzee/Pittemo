...
- (void)drawStringAt:(CGPoint)point text:(NSString*)text alignX:(AngelCodeFontAlign)aAlignX alignY:(AngelCodeFontAlign)aAlignY {
	...
	// 横アライン
	switch (aAlignX) {
		case AngelCodeFontAlignLeft:
			break;
		case AngelCodeFontAlignCenter:
			point.x -= [self getWidthForString:text] / 2.0;
			break;
		case AngelCodeFontAlignRight:
			point.x -= [self getWidthForString:text];
			break;
	}
	
	switch (aAlignY) {
		case AngelCodeFontAlignTop:
			break;
		case AngelCodeFontAlignCenter:
			point.y += [self getHeightForString:text] / 2.0;
			break;
		case AngelCodeFontAlignBottom:
			point.y += [self getHeightForString:text];
			break;
	}
...
//	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    ...
    glColor4f(alpha * colourFilter.red, alpha * colourFilter.green, alpha * colourFilter.blue, alpha * colourFilter.alpha);
...
- (void)setColourFilterRed:(float)red green:(float)green blue:(float)blue alpha:(float)aAlpha {
