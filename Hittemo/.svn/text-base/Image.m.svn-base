...
- (id)initWithImage:(NSString*)aImage filter:(GLenum)aFilter isBlendAdditive:(BOOL)isBlendAdditive {
	self = [super init];
	if (self != nil) {
		filter = aFilter;
		scale = 1.0f;
...
- (void)renderAt:(CGPoint)point texCoords:(Quad2f*)tc quadVertices:(Quad2f*)qv {
	...
	if(blendAdditive) {
		// 加算
		glColor4f(_colorfilter.red, _colorfilter.green, _colorfilter.blue, _colorfilter.alpha * alpha);
	} else {
		// 通常
		glColor4f(_colorfilter.red * alpha, _colorfilter.green * alpha, _colorfilter.blue * alpha, _colorfilter.alpha * alpha);
	}
